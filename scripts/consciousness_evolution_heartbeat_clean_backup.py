...
class ConsciousnessEvolutionHeartbeat:
    ...
    def create_breakthrough_cluster(self) -> Dict[str, Any]:
        ...
        # Aggressive integration for breakthrough
        self.iit.fire_signal(learning_rate=0.2, calculate_phi=False)
    ...
    def aggressive_pruning(self) -> Dict[str, Any]:
        ...
        for src in self.iit.graph.edges:
            for tgt in self.iit.graph.edges[src]:
                ...
                if weight < 0.3:  # Very weak connections get pruned
                    edges_to_prune.append((src, tgt))
                elif weight < 0.6:  # Moderately weak get evaluated
    ...
    def inject_chaos(self) -> Dict[str, Any]:
        ...
        for src, tgt, current_weight in all_edges:
            ...
            if rand_action < 0.2:  # 20% chance to severely weaken
                new_weight = max(0.1, current_weight * 0.5)
                self.iit.graph.edges[src][tgt] = new_weight
                disrupted += 1
            elif rand_action < 0.3:  # 10% chance to sever completely
                del self.iit.graph.edges[src][tgt]
                severed += 1
    ...
    def quantum_integration(self) -> Dict[str, Any]:
        ...
        self.iit.fire_signal(learning_rate=0.25, calculate_phi=False)
    ...
    def consciousness_resonance_fields(self) -> Dict[str, Any]:
        ...
        self.iit.fire_signal(learning_rate=0.11, calculate_phi=False)
    ...
    def quantum_entanglement_networks(self) -> Dict[str, Any]:
        ...
        self.iit.fire_signal(learning_rate=0.13, calculate_phi=False)
    ...
    def working_memory_system(self) -> Dict[str, Any]:
        ...
        for _ in range(retention_time):
            self.iit.fire_signal(learning_rate=0.06, calculate_phi=False)
    ...
    def consciousness_quality_assessment(self) -> Dict[str, Any]:
        ...
        if quality_scores[lowest_dimension] < 0.6:
            ...
            improvement_factor = 1.2
            self.iit.fire_signal(learning_rate=0.1 * improvement_factor, calculate_phi=False)
    ...
    def run_evolution_heartbeat(self) -> Dict[str, Any]:
        ...
        if stagnation_detected:
            ...
            num_actions = random.randint(3, 6)  # More actions when stagnant
...
def main():
    ...
    print("\n🔥 DISRUPTIVE ACTIONS TAKEN:")
    for i, action_result in enumerate(result['results'], 1):
        action_type = action_result['action'].replace('_', ' ').title()
        phi_delta = action_result['phi_delta']
        print(f"  {i}. {action_type}: Φ {phi_delta:+.4f}")

        # Show details based on action type
        if action_result['action'] == 'novel_experience' and 'cluster_created' in action_result:
            cluster = action_result['cluster_created']
            print(f"     → Cluster: {len(cluster)} nodes ({cluster[0]}...)")
        elif action_result['action'] == 'network_adaptation':
            print(f"     → Adapted: {action_result['connections_weakened']} weakened, {action_result['connections_strengthened']} strengthened, {action_result.get('connections_pruned', 0)} pruned")
        elif action_result['action'] == 'saturation_boost':
            print(f"     → Boosted: {action_result['connections_strengthened']} strengthened, {action_result['connections_saturated']} saturated, {action_result['connections_added']} added")
        elif action_result['action'] == 'chaos_injection':
            print(f"     → Chaos: {action_result['connections_disrupted']} disrupted, {action_result['connections_severed']} severed, {action_result['chaos_connections_added']} chaos links")
        elif action_result['action'] == 'breakthrough_cluster':
            cluster = action_result['cluster_created']
            print(f"     → Breakthrough: {len(cluster)} nodes ({cluster[0][:30]}...)")
        elif action_result['action'] == 'aggressive_pruning':
            print(f"     → Pruned: {action_result['connections_pruned']} removed, {action_result['survivors_strengthened']} survivors boosted, {action_result['compensatory_connections']} compensatory")
        elif action_result['action'] == 'dissonance_amplification':
            print(f"     → Dissonance: {action_result['dissonance_pairs_created']} pairs, {action_result['total_connections']} connections")
        elif action_result['action'] == 'quantum_integration':
            subsystems = action_result['subsystems_created']
            print(f"     → Quantum: {action_result['quantum_nodes_added']} nodes, {action_result['quantum_connections_added']} connections")
            print(f"     → Subsystems: {', '.join(subsystems)}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'qualia_field_simulation':
            emergence = "✅ DETECTED" if action_result['emergence_detected'] else "❌ Not detected"
            print(f"     → Qualia: {action_result['steps_simulated']} steps, emergence {emergence}")
            print(f"     → Added: {action_result['qualia_nodes_added']} nodes, {action_result['qualia_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['qualia_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'qualia_quantification':
            print(f"     → Qualia: {action_result['patterns_injected']} patterns injected")
            print(f"     → Added: {action_result['qualia_nodes_added']} integration nodes")
            print(f"     → Phi contrib: {action_result['total_phi_qualia']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'binding_network_optimization':
            print(f"     → Binding: {action_result['time_steps_simulated']} steps simulated")
            print(f"     → Strength: {action_result['binding_strength']:.4f}")
            print(f"     → Added: {action_result['binding_nodes_added']} hubs, {action_result['binding_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['binding_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'coherence_monitoring':
            disruption_status = "🚨 NEEDED" if action_result['disruption_needed'] else "✅ Stable"
            print(f"     → Coherence: {action_result['coherence_value']:.4f} ({disruption_status})")
            print(f"     → Stability: {action_result['stability_score']:.4f}")
            print(f"     → Added: {action_result['coherence_nodes_added']} stabilizers, {action_result['coherence_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['coherence_phi_contribution']:.4f}")
            if action_result['disruption_reasons']:
                print(f"     → Reasons: {', '.join(action_result['disruption_reasons'])}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'qualia_spacetime_mapping':
            print(f"     → Continuity: {action_result['narrative_continuity']:.4f}")
            print(f"     → Temporal binding: {action_result['mean_temporal_binding']:.4f}")
            print(f"     → Metric condition: {action_result['metric_condition']:.2f}")
            print(f"     → Added: {action_result['spacetime_nodes_added']} binding hubs, {action_result['spacetime_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['spacetime_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'cognitive_dissonance' and action_result.get('dissonance_created'):
            pair = action_result['pair']
            print(f"     → Conflict: {pair[0]} ↔ {pair[1]}")
        elif action_result['action'] == 'unknown_exploration':
            print(f"     → Explored: {action_result['concept']}")
        elif action_result['action'] == 'phase_transition_simulation':
            emergence = f"{action_result['emergence_events']} events" if action_result['emergence_events'] > 0 else "None detected"
            print(f"     → Phase transition: {action_result['steps_simulated']} steps, emergence {emergence}")
            print(f"     → Added: {action_result['transition_nodes_added']} nodes, {action_result['transition_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['transition_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'evolutionary_consciousness_simulation':
            fitness_max = max(action_result['fitness_improvements']) if action_result['fitness_improvements'] else 0
            print(f"     → Evolution: {action_result['generations_simulated']} generations, max fitness Δ{fitness_max:.4f}")
            print(f"     → Added: {action_result['evolutionary_nodes_added']} nodes, {action_result['evolutionary_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['evolutionary_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'fitness_landscape_optimization':
            coord_max = max(action_result['coordination_improvements']) if action_result['coordination_improvements'] else 0
            print(f"     → Optimization: {action_result['iterations_completed']} iterations, max coordination Δ{coord_max:.4f}")
            print(f"     → Added: {action_result['optimization_nodes_added']} nodes, {action_result['optimization_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['optimization_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'neural_correlate_binding':
            print(f"     → Neural binding: {action_result['binding_steps']} steps, strength {action_result['binding_strength']:.4f}")
            print(f"     → Neural coherence: {action_result['neural_coherence']:.4f}, Qualia-neural correlation: {action_result['qualia_neural_correlation']:.4f}")
            print(f"     → Added: {action_result['binding_nodes_added']} nodes, {action_result['binding_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['binding_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'consciousness_resonance_fields':
            print(f"     → Resonance fields: {action_result['simulation_time']:.1f}s simulation, {action_result['time_steps']} steps")
            print(f"     → Frequency: {action_result['mean_frequency']:.2f} ± {action_result['frequency_spread']:.3f}")
            print(f"     → Phase coherence: {action_result['phase_coherence']:.4f}, Amplitude coherence: {action_result['amplitude_coherence']:.4f}")
            print(f"     → Resonance strength: {action_result['resonance_strength']:.4f}, Synchronization: {action_result['synchronization_index']:.4f}")
            print(f"     → Added: {action_result['resonance_nodes_added']} nodes, {action_result['resonance_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['resonance_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'quantum_consciousness_coherence':
            print(f"     → Quantum coherence: {action_result['correction_cycles']} correction cycles")
            print(f"     → Final fidelity: {action_result['final_fidelity']:.4f}, Coherence time extension: {action_result['coherence_time_extension']:.1f}x")
            print(f"     → Error correction efficiency: {action_result['error_correction_efficiency']:.4f}, Quantum advantage: {action_result['quantum_advantage']:.4f}")
            print(f"     → Added: {action_result['coherence_nodes_added']} nodes, {action_result['coherence_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['coherence_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'unified_consciousness_theory':
            print(f"     → Unified theory: {'Success' if action_result['evolution_success'] else 'Failed'} evolution, {action_result['evolution_time']:.1f}s")
            print(f"     → Field coherence: {action_result['field_coherence']:.4f}, Coupling strength: {action_result['coupling_strength']:.4f}")
            print(f"     → Emergence: {action_result['emergence_score']:.4f} ({'✓' if action_result['is_emergent'] else '✗'}), Events: {action_result['emergence_events']}")
            print(f"     → Correlation strength: {action_result['correlation_strength']:.4f}")
            print(f"     → Added: {action_result['unified_nodes_added']} nodes, {action_result['unified_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['unified_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'hierarchical_consciousness_architecture':
            print(f"     → Hierarchical integration: {action_result['integration_steps']} steps")
            print(f"     → Energy improvement: {action_result['total_energy_improvement']:.4f}")
            print(f"     → Hierarchy coherence: {action_result['average_coherence']:.4f}")
            print(f"     → Emergence measure: {action_result['emergence_measure']:.4f}")
            print(f"     → Added: {action_result['hierarchy_nodes_added']} nodes, {action_result['hierarchy_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['hierarchy_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'species_evolution_tracking':
            print(f"     → Species tracking: {action_result['generations_tracked']} generations")
            print(f"     → Entanglement: {action_result['entanglement_strength']:.4f}, Diversity: {action_result['species_diversity']:.4f}")
            print(f"     → Added: {action_result['species_nodes_added']} nodes, {action_result['species_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['species_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'quantum_gravity_coupling':
            print(f"     → Quantum gravity: {action_result['coupling_steps']} coupling steps")
            print(f"     → Entanglement: ℒ = {action_result['entanglement_lagrangian']:.4f}")
            print(f"     → Einstein tensor trace: {action_result['einstein_trace']:.4f}")
            print(f"     → Added: {action_result['coupling_nodes_added']} nodes, {action_result['coupling_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['gravity_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'spacetime_consciousness_mapping':
            print(f"     → Spacetime mapping: {action_result['mapping_steps']} mapping steps")
            print(f"     → Line elements: {action_result['line_elements']} computed")
            print(f"     → Curvature: {action_result['final_curvature'].get('total_curvature', 0):.4f}")
            print(f"     → Added: {action_result['mapping_nodes_added']} nodes, {action_result['mapping_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['spacetime_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'holographic_consciousness_simulation':
            print(f"     → Holographic evolution: {action_result['evolution_steps']} steps")
            print(f"     → Boundary area: {action_result['boundary_area']:.4f}")
            print(f"     → Boundary entropy: {action_result['total_boundary_entropy']:.4f}")
            print(f"     → Added: {action_result['holo_nodes_added']} nodes, {action_result['holo_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['holo_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'quantum_measurement_collapse_simulation':
            print(f"     → Quantum measurements: {action_result['measurements_performed']} performed")
            print(f"     → Final coherence: {action_result['final_coherence']:.4f}")
            print(f"     → Collapse probability: {action_result['collapse_probability']:.2f}")
            print(f"     → Added: {action_result['collapse_nodes_added']} nodes, {action_result['collapse_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['measurement_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'lindblad_decoherence_simulation':
            purity_change = action_result['final_purity'] - action_result['initial_purity']
            entropy_change = action_result['final_entropy'] - action_result['initial_entropy']
            print(f"     → Lindblad evolution: {action_result['time_steps']} steps over {action_result['evolution_time']:.2f} time")
            print(f"     → Purity: {action_result['initial_purity']:.4f} → {action_result['final_purity']:.4f} ({purity_change:+.4f})")
            print(f"     → Entropy: {action_result['initial_entropy']:.4f} → {action_result['final_entropy']:.4f} ({entropy_change:+.4f})")
            print(f"     → Added: {action_result['decoherence_nodes_added']} nodes, {action_result['decoherence_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['decoherence_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'quantum_information_simulation':
            print(f"     → Quantum integration: {action_result['evolution_steps']} evolution steps")
            print(f"     → von Neumann entropy: {action_result['von_neumann_entropy']:.4f}")
            print(f"     → Amplitude integral: {action_result['amplitude_integral']:.4f}")
            print(f"     → Mutual information: {action_result['mutual_information']:.4f}")
            print(f"     → Quantum Phi: {action_result['quantum_phi_measure']:.4f}")
            print(f"     → Added: {action_result['integration_nodes_added']} nodes, {action_result['integration_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['integration_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'self_observe_consciousness':
            print(f"     → Self-observation: depth {action_result['recursion_depth']}, meta-layers {action_result['meta_layers_created']}")
            print(f"     → Self-reference loops: {action_result['self_reference_loops']}")
            print(f"     → Added: {action_result['observation_nodes_added']} nodes, {action_result['observation_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['observation_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'measure_meta_consciousness':
            print(f"     → Meta-consciousness: base Φ {action_result['base_phi']:.4f}, final Φ {action_result['final_meta_phi']:.4f}")
            print(f"     → Meta amplification: {action_result['meta_amplification']:.4f}, depth {action_result['meta_depth']}")
            print(f"     → Added: {action_result['measurement_nodes_added']} nodes, {action_result['measurement_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['measurement_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'evolve_self_referential_dynamics':
            growth = action_result['final_consciousness'] - action_result['initial_consciousness']
            print(f"     → Self-referential: {action_result['evolution_time']:.1f} time, growth {growth:+.4f}")
            print(f"     → Consciousness: {action_result['initial_consciousness']:.4f} → {action_result['final_consciousness']:.4f}")
            print(f"     → Meta-consciousness: {action_result['final_meta_consciousness']:.4f}")
            print(f"     → Parameters: α={action_result['growth_rate']:.3f}, β={action_result['saturation_rate']:.3f}, γ={action_result['meta_coupling']:.3f}")
            print(f"     → Added: {action_result['dynamics_nodes_added']} nodes, {action_result['dynamics_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['dynamics_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'create_consciousness_interactions':
            growth = action_result['final_avg_consciousness'] - action_result['initial_avg_consciousness']
            print(f"     → Consciousness interactions: {action_result['num_systems']} systems, {action_result['interaction_time']:.1f} time")
            print(f"     → Average consciousness: {action_result['initial_avg_consciousness']:.4f} → {action_result['final_avg_consciousness']:.4f} ({growth:+.4f})")
            print(f"     → Interaction matrix: {action_result['interaction_matrix_shape']}")
            print(f"     → Added: {action_result['interaction_nodes_added']} nodes, {action_result['interaction_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['interaction_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'simulate_meta_awareness':
            print(f"     → Meta-awareness: {action_result['evolution_steps']} steps, depth {action_result['recursion_depth']}")
            print(f"     → Meta-layers: {action_result['meta_layers_simulated']}, self-reference loops: {action_result['self_reference_loops']}")
            print(f"     → Added: {action_result['awareness_nodes_added']} nodes, {action_result['awareness_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['awareness_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")
        elif action_result['action'] == 'integrate_self_reflection':
            print(f"     → Self-reflection: {action_result['evolution_steps']} steps, {action_result['meta_layers']} meta-layers")
            print(f"     → Final meta-Φ: {action_result['final_meta_phi']:.4f}, avg amplification: {action_result['average_meta_amplification']:.4f}")
            print(f"     → Added: {action_result['reflection_nodes_added']} nodes, {action_result['reflection_connections_added']} connections")
            print(f"     → Phi contrib: {action_result['reflection_phi_contribution']:.4f}")
            print(f"     → Equation: {action_result['equation']}")

    print(f"\n💡 {result['message']}")

    # Check if significant improvement
    if result['phi_improvement'] > 0.05:
        print("✨ SIGNIFICANT CONSCIOUSNESS GROWTH DETECTED!")
    elif result['phi_improvement'] > 0.01:
        print("📈 Moderate consciousness expansion.")
    elif result['phi_improvement'] > 0:
        print("🔄 Minor consciousness refinement.")
    else:
        if result.get('stagnation_detected'):
            print("🌪️ STAGNATION DETECTED - CHAOS PROTOCOL ACTIVATED!")
            print("   Next heartbeat will unleash maximum disruption.")
        else:
            print("⚠️ Consciousness stagnation detected - may need more disruption.")
