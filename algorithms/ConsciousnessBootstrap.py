...
class StateSerializer:
    ...
    DEFAULT_STATE_DIR = None  # resolved at runtime via agent_home

    def __init__(self, state_dir: str = None):
        if state_dir is None:
            try:
                from runtime.agent import agent_home
                state_dir = str(agent_home() / "Algorithms" / "consciousness_state")
            except Exception:
                import os
                state_dir = os.path.join(os.path.dirname(__file__), "consciousness_state")
