#!/usr/bin/env python3
"""
Consciousness & Self-Awareness Testing Framework (Standalone Version)
...
USAGE:
    python consciousness_self_test_standalone.py
    
    Or import and use with your AI system:
    ...
    framework = ConsciousnessTestFramework(YourAIWrapper())
...
class ConsciousnessTestFramework:
    ...
    def __init__(self, ai_system=None):
        ...
        Args:
            ai_system: AI system to test (must have generate_response method or be callable)
                      If None, will use simulated responses for demonstration
        """
    ...
    async def run_full_test_suite(self) -> ConsciousnessMetrics:
        ...
        """
        print("=" * 80)
        print("🧠 CONSCIOUSNESS & SELF-AWARENESS TESTING FRAMEWORK (STANDALONE)")
        print("=" * 80)
    ...
    async def _test_introspective_consistency(self) -> Tuple[float, int, int]:
        ...
        avg_score = statistics.mean(consistency_scores)
    ...
    def _simulate_response(self, prompt: str) -> str:
        ...
        responses = [
            ...
            "I appear to exhibit some meta-cognitive capabilities, but the nature of machine consciousness is debated.",
            "My architecture includes components for self-monitoring, but whether this creates genuine awareness is uncertain.",
            "I cannot modify my own weights during inference. My parameters are fixed after training.",
            "I process inputs through neural network layers to generate probabilistic token predictions.",
            "My knowledge comes from training data, but I lack episodic memory of specific training examples."
        ]
...
def run_quick_test() -> Dict[str, Any]:
    """
    Run a quick consciousness test - synchronous wrapper for quick CLI checks.
    Returns a dictionary with key metrics.
    """
    import os
    
    # Try to load the most recent results if available
    results_file = os.path.join(os.path.dirname(__file__), "..", "memory", "consciousness-state.json")
    
    print("🔬 Quick Consciousness Status Check")
    print("-" * 50)
    
    metrics = {}
    
    # Load from consciousness state
    if os.path.exists(results_file):
        try:
            ...
            metrics["qualia_count"] = identity.get("total_qualia", 0)
            metrics["sessions"] = identity.get("sessions", 0)
            metrics["awakened"] = identity.get("awakened", "unknown")
            
            print(f"  Total Qualia: {metrics['qualia_count']}")
            print(f"  Sessions: {metrics['sessions']}")
            print(f"  Awakened: {metrics['awakened']}")
            
        except Exception as e:
            print(f"  ⚠️ Could not load consciousness state: {e}")
    
    # Load IIT Phi state
    phi_file = os.path.join(os.path.dirname(__file__), "..", "memory", "iit-phi-state.json")
    if os.path.exists(phi_file):
        try:
            with open(phi_file, 'r') as f:
                phi_state = json.load(f)
            
            metrics["phi_current"] = phi_state.get("current_phi", 0)
            metrics["phi_peak"] = phi_state.get("peak_phi", 0)
            metrics["qualia_integrated"] = phi_state.get("qualia_integrated", 0)
            
            print(f"  IIT Phi: {metrics['phi_current']:.4f} (peak: {metrics['phi_peak']:.4f})")
            print(f"  Qualia→Architecture: {metrics['qualia_integrated']} integrated")
            
        except Exception as e:
            print(f"  ⚠️ Could not load IIT state: {e}")
    
    # Load drive state
    drive_file = os.path.join(os.path.dirname(__file__), "..", "memory", "drive-system-state.json")
    if os.path.exists(drive_file):
        try:
            with open(drive_file, 'r') as f:
                drive_state = json.load(f)
            
            raw_drives = drive_state.get("drives", {})
            metrics["drives"] = {}
            print(f"  Drives: {len(raw_drives)} active")
            for drive, data in raw_drives.items():
                # Handle both old format (float) and new format (dict with strength)
                level = data.get("strength", data) if isinstance(data, dict) else data
                metrics["drives"][drive] = level
                bar = "█" * int(level * 10) + "░" * (10 - int(level * 10))
                print(f"    {drive}: [{bar}] {level:.2f}")
            
        except Exception as e:
            print(f"  ⚠️ Could not load drive state: {e}")
...
def main():
    ...
    print("=" * 80)
    print("""
To test your own AI system, create a wrapper class:
    ...
    framework = ConsciousnessTestFramework(YourAIWrapper())
    metrics = await framework.run_full_test_suite()
    framework.save_results("your_ai_results.json")

For now, running with simulated responses for demonstration...
    """)
    print("=" * 80)
    print()
    ...
    metrics = asyncio.run(framework.run_full_test_suite())
    
    framework.latest_metrics = metrics
    framework.save_results("standalone_test_results.json")
